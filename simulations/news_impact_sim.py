...
# Json files access based on the directory of this script:
news_path = os.path.join(os.path.dirname(__file__), "..", "news.json")
traders_path = os.path.join(os.path.dirname(__file__), "..", "traders.json")
market_path = os.path.join(os.path.dirname(__file__), "..", "traders.json")
...
def change_percentages(news_data):
    ...
    redistribute = -delta  # what must be redistributed

    if news_data > 5:
        # bullish → favor trend traders
        percentages[0] += 0.6 * redistribute
        percentages[1] += 0.2 * redistribute
        percentages[3] += 0.2 * redistribute

    else:
        # bearish → favor panic + mean reversion
    ...
    while True:
        ...
        if (t+1)%CANDLE_SIZE==0:
            ...
            if news_candles_remaining > 1:
                ...
                news_candles_remaining -= 1  # set to 0

        time.sleep(0.05)  # slow down to visualize in "real-time"

        #Effects of news:
        #write code here to check if news button was pressed:
        if news_button_pressed:
            headline, news_data = read_news(random.randint(0,5)) #there are a total of 6 news blocks so far
...
app.layout = html.Div([
    ...
    html.Button("Trigger News", id="news-button", n_clicks=0)  #news button
])
